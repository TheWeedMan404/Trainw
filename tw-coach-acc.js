...
function coachGymScopedQuery(query,column='gym_id',mode='match'){if(currentGymId)return query.eq(column,currentGymId);if(mode==='all')return query;return query.is(column,null)}
async function loadCoachGymMemberships(){if(!currentUserId)return[];const membershipsRes=await Trainw.api.run(sb.from('gym_memberships').select('gym_id').eq('user_id',currentUserId).eq('status','active').eq('portal','coach'),{context:'load coach gym memberships',fallback:[]});const gymIds=Array.from(new Set((membershipsRes.data||[]).map(item=>item.gym_id).filter(Boolean)));if(!gymIds.length){coachGymMemberships=[];return coachGymMemberships}const gymsRes=await Trainw.api.run(sb.from('gyms').select('id,name').in('id',gymIds),{context:'load coach gyms',fallback:[]});const gymMap=new Map((gymsRes.data||[]).map(gym=>[gym.id,gym.name||'Gym']));coachGymMemberships=gymIds.map(gymId=>({gym_id:gymId,name:gymMap.get(gymId)||'Gym'})).sort((a,b)=>String(a.name||'').localeCompare(String(b.name||''),undefined,{sensitivity:'base'}));return coachGymMemberships}
function resolveActiveCoachGymId(fallbackGymId=null){const storedGymId=localStorage.getItem('trainw_active_gym')||'';const gymIds=new Set((coachGymMemberships||[]).map(item=>item.gym_id));if(storedGymId&&gymIds.has(storedGymId))return storedGymId;if(fallbackGymId&&gymIds.has(fallbackGymId))return fallbackGymId;return coachGymMemberships[0]?.gym_id||fallbackGymId||null}
function renderCoachGymSwitcher(){const host=$('#coach-gym-switcher');if(!host)return;if((coachGymMemberships||[]).length<=1){host.classList.add('hidden');host.innerHTML='';return}host.classList.remove('hidden');host.innerHTML=`<label class="coach-gym-switcher-label" for="coach-gym-select">${Trainw.escapeHTML(copy('Salle active','Active gym','الصالة الحالية'))}</label><select class="form-input coach-gym-switcher-select" id="coach-gym-select">${coachGymMemberships.map(item=>`<option value="${item.gym_id}" ${item.gym_id===currentGymId?'selected':''}>${Trainw.escapeHTML(item.name||'Gym')}</option>`).join('')}</select>`}
function handleCoachGymSwitch(gymId){if(!gymId||gymId===currentGymId)return;currentGymId=gymId;currentProfile.gym_id=currentGymId;localStorage.setItem('trainw_active_gym',gymId);runCoachReload()}
function syncCoachFreelanceNav(){const freelance=window.__coachFreelanceMode===true;['checkin','analytics'].forEach(page=>{$$(`.nav-item[data-page="${page}"]`).forEach(item=>item.classList.toggle('hidden',freelance))});if(freelance&&$$('.nav-item.hidden.active').length)showPage('dashboard')}
function renderCoachFreelanceBanner(){const existing=document.querySelector('[data-coach-freelance-banner]');if(existing)existing.remove();const host=$('#coach-dashboard-page')||$('#dashboard-page')||document.querySelector('.main-content');if(!host||!window.__coachFreelanceMode||window.__coachFreelanceBannerDismissed)return;const banner=document.createElement('div');banner.dataset.coachFreelanceBanner='true';banner.className='card coach-freelance-banner';banner.style.marginBottom='18px';banner.innerHTML=`<div style="display:flex;align-items:flex-start;justify-content:space-between;gap:16px;flex-wrap:wrap;"><div><div class="card-title" style="margin-bottom:8px;">Mode Ind&eacute;pendant</div><p class="card-subtitle" style="margin:0;">Vous n&apos;&ecirc;tes affili&eacute; &agrave; aucune salle. Vos donn&eacute;es restent priv&eacute;es.</p></div><div class="page-head-actions" style="margin:0;flex-wrap:nowrap;"><button class="btn-primary" type="button" data-freelance-join>Rejoindre une salle &rarr;</button><button class="btn-secondary" type="button" data-freelance-dismiss aria-label="Fermer">&times;</button></div></div>`;host.insertBefore(banner,host.firstChild);banner.querySelector('[data-freelance-dismiss]')?.addEventListener('click',()=>{window.__coachFreelanceBannerDismissed=true;banner.remove()});banner.querySelector('[data-freelance-join]')?.addEventListener('click',()=>{showPage('profile');$('#profile-page')?.scrollIntoView({behavior:'smooth',block:'start'})})}
function syncCoachFreelanceModeUI(){if(!window.__coachFreelanceMode)window.__coachFreelanceBannerDismissed=false;syncCoachFreelanceNav();renderCoachFreelanceBanner()}
...
function subscribeCoachRealtime(){clearCoachRealtime();if(!currentUserId)return;const add=ch=>{coachRealtimeChannels.push(ch);return ch};add(sb.channel(`coach-sessions-${currentUserId}`).on('postgres_changes',{event:'*',schema:'public',table:'sessions',filter:`coach_id=eq.${currentUserId}`},()=>queueCoachReload(220)).subscribe());if(!currentGymId)return;add(sb.channel(`coach-msg-${currentUserId}`).on('postgres_changes',{event:'*',schema:'public',table:'messages',filter:`gym_id=eq.${currentGymId}`},payload=>{const row=payload.new||payload.old||{};if(row.sender_id===currentUserId||row.receiver_id===currentUserId)queueCoachReload(180)}).subscribe());add(sb.channel(`coach-checkins-${currentUserId}`).on('postgres_changes',{event:'*',schema:'public',table:'check_ins',filter:`gym_id=eq.${currentGymId}`},()=>queueCoachReload(260)).subscribe());add(sb.channel(`coach-weights-${currentUserId}`).on('postgres_changes',{event:'*',schema:'public',table:'weight_logs',filter:`gym_id=eq.${currentGymId}`},()=>queueCoachReload(260)).subscribe())}
async function bootstrap(){
  const rememberedGymId=localStorage.getItem('trainw_active_gym')||'';
  ...
  await loadCoachGymMemberships();
  if(rememberedGymId&&coachGymMemberships.some(item=>item.gym_id===rememberedGymId))localStorage.setItem('trainw_active_gym',rememberedGymId);
  const inferredGymId=currentGymId||await inferCoachGymId();
  currentGymId=resolveActiveCoachGymId(inferredGymId);
  if(currentGymId)localStorage.setItem('trainw_active_gym',currentGymId);
  else localStorage.removeItem('trainw_active_gym');

  currentProfile.gym_id=currentGymId||null;
  window.__coachFreelanceMode=!currentGymId;
  ...
  const [cp,assignsRes,sessRes,avRes,plansRes,templRes,msgRes]=await Promise.all([
    ...
    Trainw.api.run(
      coachGymScopedQuery(
        sb.from('coach_client_assignments').select('client_id').eq('coach_id',currentUserId).eq('is_active',true),
        'gym_id',
        'all'
      ),
      {context:'load assigned clients',fallback:[]}
    ),
    Trainw.api.run(
      coachGymScopedQuery(
        sb.from('sessions')
          .select('id,gym_id,client_id,session_date,start_time,end_time,duration_minutes,status,session_name,notes,cancellation_reason,booked_at,confirmed_at,completed_at,coach_id,type,client:users!sessions_client_id_fkey(id,name,phone,email)')
      ...
      {context:'load coach sessions',fallback:[]}
    ),
    Trainw.api.run(
      coachGymScopedQuery(
        sb.from('coach_availability')
          .select('*')
          .eq('coach_id',currentUserId)
      ...
      {context:'load coach availability',fallback:[]}
    ),
    Trainw.api.run(
      coachGymScopedQuery(
      ...
      {context:'load coach plans',fallback:[]}
    ),
    Trainw.api.run(
      coachGymScopedQuery(
        sb.from('workout_plans')
      ...
      {context:'load coach templates',fallback:[]}
    ),
    Trainw.api.run(
      coachGymScopedQuery(
        sb.from('messages')
      ...
      {context:'load coach messages',fallback:[]}
    )
  ]);
  ...
  await warmSessionEnrollmentNames(sessions);
  ...
  if(ids.length){
    const [usersRes,profilesRes,checksRes,weightsRes]=await Promise.all([
      ...
      Trainw.api.run(
        coachGymScopedQuery(
          sb.from('check_ins').select('client_id,checked_in_at,method').in('client_id',ids).order('checked_in_at',{ascending:false}),
          'gym_id',
          'all'
        ),
        {context:'load coach checkins',fallback:[]}
      ),
      Trainw.api.run(
        coachGymScopedQuery(
          sb.from('weight_logs').select('client_id,weight_kg,logged_at').in('client_id',ids).order('logged_at',{ascending:false}),
          'gym_id',
          'all'
        ),
        {context:'load coach weights',fallback:[]}
      )
    ]);
  ...
  threads=assignedClients.map(c=>({
    id:c.id,
    name:c.name,
    role:'Client',
    messages:(msgRes.data||[]).filter(m=>m.sender_id===c.id||m.receiver_id===c.id).sort((a,b)=>new Date(a.created_at)-new Date(b.created_at))
  }));
...
function renderAll(){applyTranslations();$('#sidebar-coach-name').textContent=safe(currentProfile?.name||currentProfile?.email||'Coach');renderCoachGymSwitcher();$('#coach-name-input').value=currentProfile?.name||'';$('#coach-phone-input').value=currentProfile?.phone||'';$('#coach-email-input').value=currentProfile?.email||'';$('#coach-speciality-input').value=coachProfile?.specialty||'';$('#coach-experience-input').value=coachProfile?.experience_years??'';$('#coach-rate-input').value=coachProfile?.price_per_session??coachProfile?.hourly_rate??'';$('#coach-bio-input').value=coachProfile?.bio||'';renderDashboard();renderClients();renderPrograms();renderWeightRows();renderSessions();applyCoachStructuredEmptyStates();renderMessages();renderProfileSummary();syncCoachFreelanceModeUI();lucide.createIcons()}
...
function renderCoreEmptyState(target,options){
  ...
  target.innerHTML=Trainw.ui.emptyState(options?.icon,options?.title,options?.body,options?.actionLabel||null,actionAttr);
...
function focusAvailabilityComposer(){
  showPage('sessions');
  const input=$('#availability-start-input');
  input?.scrollIntoView({behavior:'smooth',block:'center'});
  input?.focus();
}
function renderClients(){
  const list=filteredClients();
  const target=$('#coach-clients-grid');
  if(!target)return;
  if(!list.length){
    renderCoreEmptyState(target,{icon:'👥',title:copy('Aucun client assigne','No assigned clients','لا يوجد عملاء معينون'),body:copy('Les clients assignes apparaitront ici des qu ils seront relies a votre compte.','Assigned clients will appear here as soon as they are linked to your account.','سيظهر العملاء المعينون هنا فور ربطهم بحسابك.')});
...
function applyCoachStructuredEmptyStates(){
  if(!filteredClients().length)renderCoreEmptyState($('#coach-clients-grid'),{icon:'👥',title:copy('Aucun client assigne','No assigned clients','لا يوجد عملاء معينون'),body:copy('Les clients assignes apparaitront ici des qu ils seront relies a votre compte.','Assigned clients will appear here as soon as they are linked to your account.','سيظهر العملاء المعينون هنا فور ربطهم بحسابك.')});
  if(!plans.length)renderCoreEmptyState($('#coach-plan-list'),{icon:'📋',title:copy('Aucun programme','No plans yet','لا توجد برامج بعد'),body:copy('Creez un programme pour cadrer les prochaines seances de vos clients.','Create a plan to structure your clients next sessions.','أنشئ برنامجاً لتنظيم الحصص القادمة لعملائك.'),actionLabel:t('newProgram'),actionId:'coach-empty-new-plan',onAction:()=>openPlanModal()});
  if(!templates.length)renderCoreEmptyState($('#coach-template-list'),{icon:'🧩',title:copy('Aucun modele','No templates yet','لا توجد قوالب بعد'),body:copy('Enregistrez un modele reutilisable pour accelerer la creation de programmes.','Save a reusable template to speed up future plans.','احفظ قالباً قابلاً لإعادة الاستخدام لتسريع إنشاء البرامج.'),actionLabel:t('newTemplate'),actionId:'coach-empty-new-template',onAction:()=>openPlanModal('',true)});
  if(!assignedClients.length)renderCoreEmptyState($('#coach-weight-table'),{icon:'⚖️',title:copy('Aucun client assigne','No assigned clients','لا يوجد عملاء معينون'),body:copy('Ajoutez des clients a votre suivi pour voir leur progression de poids ici.','Assign clients to your roster to see their weight progress here.','أضف عملاء إلى متابعتك لرؤية تقدم الوزن هنا.')});
  if(!availability.length)renderCoreEmptyState($('#coach-availability-list'),{icon:'🗓️',title:copy('Aucune disponibilite','No availability yet','لا يوجد توفر بعد'),body:copy('Ajoutez vos creneaux pour permettre aux clients de reserver.','Add your slots so clients can book with you.','أضف فترات توفرك ليتمكن العملاء من الحجز معك.'),actionLabel:copy('Ajouter un creneau','Add slot','أضف فترة'),actionId:'coach-empty-availability',onAction:focusAvailabilityComposer});
  if(!availability.filter(a=>a.is_active).length)renderCoreEmptyState($('#coach-availability-summary'),{icon:'🗓️',title:copy('Aucune disponibilite publiee','No published availability','لا يوجد توفر منشور'),body:copy('Publiez vos horaires pour ouvrir la reservation client.','Publish your hours to enable client booking.','انشر ساعات توفرك لتفعيل حجز العملاء.'),actionLabel:copy('Ajouter un creneau','Add slot','أضف فترة'),actionId:'coach-empty-availability-summary',onAction:focusAvailabilityComposer});
}
async function warmSessionEnrollmentNames(sessionList){const groupSessionIds=Array.from(new Set((Array.isArray(sessionList)?sessionList:[]).filter(s=>['group_class','group_activity'].includes(String(s?.type||''))).map(s=>s.id).filter(Boolean)));const missingIds=groupSessionIds.filter(id=>!sessionEnrollmentNames.has(id));if(!missingIds.length)return;const enrollmentsRes=await Trainw.api.run(coachGymScopedQuery(sb.from('session_enrollments').select('session_id, users!session_enrollments_client_id_fkey(name)').in('session_id',missingIds).order('enrolled_at',{ascending:true}),'gym_id','all'),{context:'load coach session enrollments',fallback:[],toastOnError:false,silent:true});const namesBySession=new Map(missingIds.map(id=>[id,[]]));(enrollmentsRes.data||[]).forEach(row=>{const name=String(row?.users?.name||'').trim();if(!name)return;const arr=namesBySession.get(row.session_id)||[];arr.push(name);namesBySession.set(row.session_id,arr)});missingIds.forEach(id=>sessionEnrollmentNames.set(id,namesBySession.get(id)||[]))}
function renderSessionCard(s,withActions){const meta=sessionStatusMeta(s.status||'pending'),clientName=s.client?.name||assignedClients.find(c=>c.id===s.client_id)?.name||'Client';const endCopy=s.end_time?` - ${String(s.end_time).slice(0,5)}`:'';const enrolledNames=['group_class','group_activity'].includes(String(s?.type||''))?(sessionEnrollmentNames.get(s.id)||[]):[];const enrollmentLine=enrolledNames.length?`<div class="coach-session-note">${Trainw.escapeHTML(enrolledNames.join(', '))}</div>`:'';const actions=[];if(withActions&&s.client_id)actions.push(`<button class="btn-secondary" type="button" data-open-client="${s.client_id}">${Trainw.escapeHTML(t('openClient'))}</button>`);if(withActions&&s.status==='pending'){actions.push(`<button class="btn-primary" type="button" data-session-action="confirm" data-session-id="${s.id}">Confirmer</button>`);actions.push(`<button class="btn-secondary" type="button" data-session-action="cancel" data-session-id="${s.id}">Refuser</button>`)}if(withActions&&s.status==='confirmed'){if(canCompleteSession(s))actions.push(`<button class="btn-primary" type="button" data-session-action="complete" data-session-id="${s.id}">Terminer</button>`);actions.push(`<button class="btn-secondary" type="button" data-session-action="cancel" data-session-id="${s.id}">Annuler</button>`)}return `<article class="coach-session-card"><div class="coach-session-top"><div><div class="coach-session-title">${Trainw.escapeHTML(s.session_name||'Coach session')}</div><div class="coach-session-meta">${Trainw.escapeHTML(fmt(s.session_date))} Â· ${Trainw.escapeHTML(String(s.start_time||'').slice(0,5))}${Trainw.escapeHTML(endCopy)} Â· ${Trainw.escapeHTML(clientName)}</div>${enrollmentLine}${s.notes?`<div class="coach-session-note">${Trainw.escapeHTML(s.notes)}</div>`:''}${s.cancellation_reason?`<div class="coach-session-note">${Trainw.escapeHTML(s.cancellation_reason)}</div>`:''}</div><span class="coach-status-pill ${meta.cls}">${Trainw.escapeHTML(meta.label)}</span></div>${actions.length?`<div class="coach-session-actions">${actions.join('')}</div>`:''}</article>`}
...
async function openClientPanel(id){const c=assignedClients.find(x=>x.id===id);if(!c)return;const [goalRes,planRes,logsRes,msgRes]=await Promise.all([Trainw.api.run(coachGymScopedQuery(sb.from('client_goals').select('*').eq('client_id',id).order('created_at',{ascending:false}).limit(1)),{context:'client goal panel',fallback:[]}),Trainw.api.run(coachGymScopedQuery(sb.from('workout_plans').select('*').eq('client_id',id).order('updated_at',{ascending:false}).limit(1)),{context:'client plan panel',fallback:[]}),Trainw.api.run(coachGymScopedQuery(sb.from('workout_logs').select('*').eq('client_id',id).order('completed_at',{ascending:false}).limit(8)),{context:'client logs panel',fallback:[]}),Trainw.api.run(coachGymScopedQuery(sb.from('messages').select('*').or(`and(sender_id.eq.${currentUserId},receiver_id.eq.${id}),and(sender_id.eq.${id},receiver_id.eq.${currentUserId})`).order('created_at',{ascending:false}).limit(40)),{context:'client msg panel',fallback:[]})]);const plan=(planRes.data||[])[0]||null;const ex=plan?await Trainw.api.run(sb.from('workout_exercises').select('*').eq('plan_id',plan.id).order('order_index',{ascending:true}),{context:'client ex panel',fallback:[]}):{data:[]};clientPanel={id,tab:'overview',data:{client:c,goal:(goalRes.data||[])[0]||null,plan,ex:ex.data||[],logs:logsRes.data||[],messages:msgRes.data||[]},chart:null};renderClientPanel();Trainw.openPanel('coach-client-panel','coach-client-panel-backdrop')}
...
async function savePlan(asTemplate){const id=$('#workout-plan-id').value||'',payload={gym_id:currentGymId??null,created_by:currentUserId,client_id:asTemplate?null:($('#plan-client-input').value||null),title:$('#plan-title-input').value.trim()||'Plan',goal:$('#plan-goal-input').value||null,frequency_per_week:+($('#plan-frequency-input').value||0)||null,description:$('#plan-description-input').value||null,is_template:!!asTemplate};try{const planRes=id?await Trainw.api.run(sb.from('workout_plans').update(payload).eq('id',id).select('id').maybeSingle(),{context:'update plan',fallback:null}):await Trainw.api.run(sb.from('workout_plans').insert(payload).select('id').maybeSingle(),{context:'insert plan',fallback:null});if(planRes.error)throw planRes.error;if(!planRes.data?.id)throw new Error('Erreur');const planId=planRes.data.id;const clearRes=await Trainw.api.run(sb.from('workout_exercises').delete().eq('plan_id',planId),{context:'clear exercises',fallback:null});if(clearRes.error)throw clearRes.error;const rows=[];$$('#plan-days-builder .plan-day-card').forEach((dayCard,dayIndex)=>{const label=dayCard.querySelector('[data-day-label]')?.value||`Jour ${dayIndex+1}`;dayCard.querySelectorAll('.exercise-builder-row').forEach((row,i)=>{rows.push({plan_id:planId,day_label:label,exercise_name:row.querySelector('[data-ex-name]')?.value||'Exercise',sets:+(row.querySelector('[data-ex-sets]')?.value||0)||null,reps:row.querySelector('[data-ex-reps]')?.value||null,rest_seconds:+(row.querySelector('[data-ex-rest]')?.value||0)||null,notes:row.querySelector('[data-ex-notes]')?.value||null,order_index:i})})});if(rows.length){const saveExercisesRes=await Trainw.api.run(sb.from('workout_exercises').insert(rows),{context:'save exercises',fallback:null});if(saveExercisesRes.error)throw saveExercisesRes.error}Trainw.ui.showToast(t('planSaved'),'success');$('#workout-plan-modal').classList.remove('show');await bootstrap()}catch(error){Trainw.ui.showToast(error?.message||'Erreur','error')}}
async function saveInlineWeight(clientId){const val=+($(`[data-weight-input="${clientId}"]`)?.value||0);if(!Number.isFinite(val)||val<=0){Trainw.ui.showToast(t('weightKg'),'info');return}const btn=$(`[data-save-weight="${clientId}"]`)||$('#btn-save-client-weight');setCoachBusy(btn,true);try{const r=await Trainw.api.run(sb.from('weight_logs').insert({client_id:clientId,gym_id:currentGymId??null,logged_by:currentUserId,weight_kg:val,logged_at:new Date().toISOString()}),{context:'coach save weight',fallback:null});if(r.error)return;Trainw.ui.showToast(t('weightSaved'),'success');await runCoachReload();const c=assignedClients.find(x=>x.id===clientId);if(c)drawClientWeight(c)}finally{setCoachBusy(btn,false)}}
async function saveAvailability(){const day=+($('#availability-day-input').value||0),start=$('#availability-start-input').value,end=$('#availability-end-input').value,slot=+($('#availability-slot-input').value||60);if(!start||!end||start>=end){Trainw.ui.showToast(currentLang==='fr'?'Verifiez les horaires de disponibilite.':'Check the availability time range.','info');return}if(!Number.isFinite(slot)||slot<=0){Trainw.ui.showToast(currentLang==='fr'?'Duree de slot invalide.':'Invalid slot duration.','info');return}const btn=$('#btn-save-availability');setCoachBusy(btn,true);try{const r=await Trainw.api.run(sb.from('coach_availability').insert({coach_id:currentUserId,gym_id:currentGymId??null,day_of_week:day,start_time:start,end_time:end,slot_minutes:slot,is_active:true}),{context:'save coach availability',fallback:null});if(r.error)throw r.error;$('#availability-start-input').value='';$('#availability-end-input').value='';Trainw.ui.showToast(currentLang==='fr'?'Disponibilite ajoutee.':'Availability saved.','success');await runCoachReload()}catch(error){Trainw.ui.showToast(error?.message||'Erreur','error')}finally{setCoachBusy(btn,false)}}
async function removeAvailability(id){try{const r=await Trainw.api.run(sb.from('coach_availability').delete().eq('id',id),{context:'delete coach availability',fallback:null});if(r.error)throw r.error;await bootstrap()}catch(error){Trainw.ui.showToast(error?.message||'Erreur','error')}}
...
document.body.addEventListener('change',e=>{const select=e.target.closest('#coach-gym-select');if(select)handleCoachGymSwitch(select.value)});
