# TRAINW V17 Database Audit

## Scope

Reviewed the legacy SQL material carried forward or referenced during the V17 upgrade:

- `Gym Demo Reset & Seed (Coaches, Clients, Sessions).sql`
- Historical reference scripts from the earlier root bundle (`Main-Query.sql`, `Booking-System.sql`, `Booking-SLS.sql`)
...
`Gym Demo Reset & Seed (Coaches, Clients, Sessions).sql` was not structured as an idempotent seed aligned with the final schema.
